package com.example.juanpablo.lab3;
...
public class Activity3 extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX > 0) {
                        ...
                        // de Activity2 a MainActivity
                        Intent Target = new Intent(this, Activity4.class);
                        startActivity(Target);
                    } else {
                        Toast.makeText(getApplicationContext(), "Swipe Left." , Toast.LENGTH_SHORT).show();
                        // de Activity2 a MainActivity
                        Intent Target = new Intent(this, Activity2.class);
                        startActivity(Target);
                    }
                    result = true;
                }
